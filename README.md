...
Run `npx nx serve remote2` to start the remote2 development server (mfe2).
...
✨ To create **Nx workspace** angular monorepo with following cmd:

```
npx create-nx-workspace@latest my-mfe --preset=angular-monorepo
```
...
✨ To create **remote2** app and setup native federation with Nx use the following cmd:

```
npx nx g @nx/angular:app remote2 --directory=apps/remote2
```

Create Home Module and HomeComponent in remote2 app:

```
npx nx g module remote-main --project remote2
npx nx g component home --module=remote-main --directory=apps/remote2/src/app
```

Install Native federation on remote2:

```
npx nx add @angular-architects/native-federation --project remote2 --port 4202
```
...

{
  ...
  "remote2": "http://localhost:4202/remoteEntry.json",
...
[
  ...
  {
    "title": "Remote2 application",
    "description": "Remote app with native fedration",
    "path": "remote2",
    "layout": "KendoDrawerComponent",
    "remoteModule": {
      "name": "remote2",
      "exposes": "./remote2Module",
      "module": "RemoteMainModule"
    },
  ...
  const route = data.map((route: any) => {
    if (route.remoteModule)
      return {
        path: route.path,
        loadComponent: () =>
          import('@mfe-starter-kit/shared-ui').then((m: any) => m[route.layout]), // Dynamic import based on layout
        loadChildren: () =>
          loadRemoteModule(
      ...
      return {
        ...
        loadComponent: () =>
          import('@mfe-starter-kit/shared-ui').then((m: any) => m[route.layout]),
        loadChildren: async () => {
